...
export default function LoginModal({
    ...
    const handleGoogleSignIn = async () => {
        try {
            ...
            if (
                isNewUser ||
                (!user?.isOnboarded && user?.authProvider === "google")
            ) {
        ...
        <>
            ...
            Coming Soon
                    ...
                    <motion.button
                        ...
                        type="button"
                        className="w-full flex items-center justify-center gap-2 border border-white/20 bg-white/10 hover:bg-white/10 text-white py-3 px-4 rounded-lg transition-colors disabled:opacity-40"
                        whileHover={{scale: 1}}
                        whileTap={{scale: 1}}
                        onClick={handleGoogleSignIn}
                    >
                        <FcGoogle className="w-5 h-5"/>
                        <span>{isGoogleSubmitting ? "Signing in..." : "Google Sign-in"}</span>
                    </motion.button>
                </motion.div>
            </form>
